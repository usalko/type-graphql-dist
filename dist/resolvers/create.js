...
function createHandlerResolver(resolverMetadata) {
    ...
    helpers_1.applyAuthChecker(middlewares, authChecker, container, authMode, resolverMetadata.roles);
...
function createAdvancedFieldResolver(fieldResolverMetadata) {
    ...
    helpers_1.applyAuthChecker(middlewares, authChecker, container, authMode, fieldResolverMetadata.roles);
    return (root, args, context, info) => {
        const resolverData = { root, args, context, info };
...
function createBasicFieldResolver(fieldMetadata) {
    ...
    helpers_1.applyAuthChecker(middlewares, authChecker, container, authMode, fieldMetadata.roles);
    return (root, args, context, info) => {
        const resolverData = { root, args, context, info };
...
function wrapResolverWithAuthChecker(resolver, container, roles) {
    ...
    return (root, args, context, info) => {
        const resolverData = { root, args, context, info };
        return auth_middleware_1.AuthMiddleware(authChecker, container, authMode, roles)(resolverData, async () => resolver(root, args, context, info));
    };
}
